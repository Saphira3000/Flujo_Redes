        ...
        with open(str(file_name)+".plot", 'w') as archivo:
            ...
            for a in self.E:
                ...
                if pond != 0 and diri == False: # ponderado
                    ...
                    print('set arrow',i,'from', x1 , ',', y1, 'to', x2,',', y2,'nohead', file = archivo)
                    i += 1
            print('plot "' + datos + '" u 1:2:3 with circles lc "purple" fs solid 0.5, "' + datos + '" using 1:2:(sprintf("%d", $4)) with labels offset char 0,0 notitle', file = archivo)
    ...
    def per_nodo(self,cual): # percolacion nodos
        ...
        for a in vec:
            ...
            if (cual,a) in self.E:
                self.per_aris(quien)
        # para la grafica
        datos = open("nodos.dat","r")
        lineas = datos.readlines()
        elimina = lineas[cual]
        lineas.remove(elimina)
        archivo = open("nodos2.dat",'w')
