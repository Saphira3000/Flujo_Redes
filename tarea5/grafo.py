class Grafo:
        ...
        with open("grafo.plot", 'w') as archivo:
            ...
            for v in self.E:
                ...
                if v[3] != 0 and v[2] == False: # ponderado
                    ...
                    print('set arrow',i,'from', x1 , ',', y1, 'to', x2,',', y2,'nohead', file = archivo)
                    i += 1
            print('plot "nodos.dat" u 1:2:3 with circles lc "purple" fs solid 0.5, "nodos.dat" using 1:2:(sprintf("%d", $4)) with labels offset char 0,0 notitle', file = archivo)
    ...
    def man(self,a,b): # distancia del taxista (Manhatan)
        (x1, y1) = self.V[a]
        (x2, y2) = self.V[b]
        dx = abs(x1 - x2)
        dy = abs(y1 - y2)
